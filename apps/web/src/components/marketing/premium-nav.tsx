...
const LINKS = ['Analizza', 'Catalogo', 'Community', 'Prezzi']
  ...
  return (
    <div className="a-in sticky top-4 z-40 px-4">
      <nav className="glass mx-auto flex h-14 max-w-[880px] items-center justify-between rounded-full pl-5 pr-2">
        {/* logo */}
        <a href="/preview" className="group flex items-center gap-2">
        ...
        <div className="relative hidden items-center md:flex" onMouseLeave={() => setHl((h) => ({ ...h, opacity: 0 }))}>
          ...
          />
          {LINKS.map((l) => (
            <a key={l} href="#" onMouseEnter={enter} className="relative z-10 rounded-full px-4 py-2 text-[15px] font-medium text-muted transition-colors hover:text-text">
              {l}
            </a>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <button className="group inline-flex items-center gap-1.5 rounded-full bg-text px-5 py-2 text-sm font-semibold text-bg transition-transform hover:scale-[1.04]">
