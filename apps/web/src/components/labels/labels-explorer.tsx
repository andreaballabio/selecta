...
import { Search, Upload, ArrowRight, Mail, CheckCircle2 } from 'lucide-react'
...
export function LabelsExplorer({ labels }: { labels: DirLabel[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    let list = labels.filter((l) => {
      ...
      if (needle) {
        const hay = [l.name, l.primary_genre, ...(l.reference_artists ?? []), ...soundTags(l.profile)].join(' ').toLowerCase()
  ...
  return (
    <div className="space-y-6">
      {/* Porta d'ingresso: cerca a parole + upsell al match vero */}
      <div className="rounded-2xl border border-line bg-surface/60 p-4 sm:p-5">
        <div className="relative">
          <Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-faint" />
          <input
            value={q} onChange={(e) => setQ(e.target.value)}
            placeholder="Descrivi il tuo suono o cerca una label… (es. «dark punchy», «sub-heavy», «Toolroom»)"
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
          <p className="text-xs text-muted">Cerchi a parole come su un motore. <span className="text-faint">Ma il match più preciso lo dà la tua traccia →</span></p>
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
        ...
        <div className="ml-auto flex items-center gap-2 text-xs text-muted">
          <span>Ordina</span>
          <select value={sort} onChange={(e) => setSort(e.target.value as Sort)} className="rounded-lg border border-line bg-surface-2 px-2 py-1.5 text-text focus:border-accent focus:outline-none">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((l) => <LabelCard key={l.id} l={l} />)}
...
function LabelCard({ l }: { l: DirLabel }) {
  ...
  return (
    <Link href={`/labels/${l.slug || l.id}`} className="group flex flex-col gap-3 rounded-2xl border border-line bg-surface/50 p-4 transition-colors hover:border-faint">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-medium ${toneClass[dm.tone]}`}>{dm.label}</span>
      </div>
