  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {artists.map((a) => {
        ...
        return (
          <div key={a.user_id} className="rounded-2xl border border-line bg-surface/60 p-4">
            <div className="flex items-center gap-3">
              {a.photo_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={a.photo_url} alt={a.display_name ?? ''} className="h-12 w-12 rounded-xl object-cover" />
              ) : (
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-accent/30 to-cyan-500/20 font-bold text-accent">{initials}</div>
              )}
              <div className="min-w-0 flex-1">
                <Link href={a.handle ? `/u/${a.handle}` : '#'} className="block truncate font-semibold text-text hover:text-accent">
                  {a.display_name || a.handle || 'Artista'}
                </Link>
                {a.tagline && <p className="truncate text-sm text-muted">{a.tagline}</p>}
              </div>
            </div>
            ...
            {a.descriptors.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {a.descriptors.slice(0, 4).map((d) => (
                  <span key={d} className="rounded-full border border-line px-2 py-0.5 text-xs text-muted">{d}</span>
                ))}
              </div>
            )}
