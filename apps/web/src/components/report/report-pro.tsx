...
const TONE: Record<Tone, { ring: string; chip: string; icon: typeof Info; iconCls: string }> = {
  good: { ring: 'border-emerald-500/30 bg-emerald-950/20', chip: 'text-emerald-400', icon: CheckCircle2, iconCls: 'text-emerald-400' },
  ...
  info: { ring: 'border-zinc-800 bg-zinc-950/40', chip: 'text-zinc-400', icon: Info, iconCls: 'text-zinc-500' },
}

const READINESS = {
  ready: { label: 'Pronta', cls: 'text-emerald-400', bar: 'bg-emerald-500' },
    ...
    <section className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-6">
      ...
      <div className="mb-5 flex items-start justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Report PRO</h2>
            <p className="text-xs text-zinc-500">Analisi tecnica del tuo master</p>
          </div>
        </div>
      ...
      <div className="mb-6 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
        <div className="mb-2 flex items-end justify-between">
          <div>
            <p className="text-xs uppercase tracking-wider text-zinc-500">Pronta per la demo?</p>
            <p className={`text-2xl font-bold ${r.cls}`}>{r.label}</p>
          </div>
          <p className="text-3xl font-bold text-white">{report.readiness.score}<span className="text-base text-zinc-600">/100</span></p>
        </div>
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-zinc-800">
          <div className={`h-full rounded-full ${r.bar}`} style={{ width: `${report.readiness.score}%` }} />
        </div>
        <p className="mt-2 text-sm text-zinc-400">{report.readiness.headline}</p>
      </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-sm text-zinc-400">{item.verdict}</p>
                          {item.advice && (
                            <p className="mt-2 text-sm leading-relaxed text-zinc-500">{item.advice}</p>
                          )}
                        </div>
                      </div>
